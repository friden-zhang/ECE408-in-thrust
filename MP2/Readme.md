...
matrix multiplication. Thrust is still used to orchestrate GPU execution, yet
each output element is produced by a custom functor that consumes two matrices
represented as contiguous buffers. The goal is to understand how to express a
more complex operation—`C = A * B`—without writing an explicit CUDA kernel.
...
After building, launch the MP2 executable and point it at the dataset folder:

```bash
./build/MP2/mp2 -w /path/ECE408-in-thrust/MP2/data
```
...
Example:
```bash
./build/MP2/mp2 --work_dir /path/to/data
```

## Input Data Format

Each subdirectory in `MP2/data` contains three `.raw` files that describe
...
```
Found sub-directory: "/path/ECE408-in-thrust/MP2/data/0"
...
* `MatrixMulOp` functor with `cuda::std::mdspan` views to compute each output
  element
* `thrust::transform` over a counting iterator to launch `M * P` evaluations
